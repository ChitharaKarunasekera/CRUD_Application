...
public class MainController implements Initializable {
    ...
    @FXML
    void handleButtonAction(ActionEvent event) {//Checks the button
        if(event.getSource() == btnInsert)
        {
            insertRecord();//Calls the method that insert a query
        }
        else if(event.getSource() == btnUpdate)
        {
            updateRecord();//Calls the method that updates a query
        }
        else if(event.getSource() == btnDelete)
        {
            deleteButton();//Deletes the record
        }
    ...
    private void insertRecord(){
        String query = "insert INTO books VALUES (" + tfId.getText() + ",'" + tfTitle.getText() + "','" + tfAuthor.getText() + "',"
                + tfYear.getText() + "," + tfPages.getText() + ")";//Inserting text to DB.
